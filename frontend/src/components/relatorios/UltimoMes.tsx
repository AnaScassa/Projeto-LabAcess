    ...
    <div className="card-body p-0">
      <div className="table-responsive">
        <table className="table table-hover mb-0">
          <thead className="table-light">
            <tr>
              <th className="px-4 py-3 text-secondary small text-uppercase fw-semibold" style={{cursor: "pointer"}} onClick={() => {if (sortFieldPoucoTempo === "nome") 
                {setSortAscPouco((prev) => !prev);} else {setSortFieldPoucoTempo("nome"); setSortAscPouco(true);} setPage(0);}}>
                Usuário {sortFieldPoucoTempo === "nome" ? (sortAscPouco ? "▲" : "▼") : ""}
              </th>

              <th className="px-4 py-3 text-secondary small text-uppercase fw-semibold" style={{cursor: "pointer"}} onClick={() => {if (sortFieldPoucoTempo === "tempoTotal") 
                {setSortAscPouco((prev) => !prev);} else {setSortFieldPoucoTempo("tempoTotal"); setSortAscPouco(false);} setPage(0);}}>
      ...
      <div className="d-flex flex-wrap align-items-center justify-content-between gap-3 px-4 py-3 border-top">
        <div className="text-secondary small">
          Mostrando {relatorioOrdenado.length === 0 ? 0 : page * rowsPerPage + 1}{" "}
          a {Math.min((page + 1) * rowsPerPage, relatorioOrdenado.length)}{" "}
          de {relatorioOrdenado.length} registros
        </div>
